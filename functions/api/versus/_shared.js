...
export const VERSUS_CATEGORIES = ["flags", "capitals"];
export const VERSUS_CONTINENTS = ["Monde entier", "Afrique", "Europe", "Asie", "Amérique", "Océanie"];
...
export function cleanCode(value) { return String(value || "").trim().toUpperCase().slice(0, 10); }
export function cleanPseudo(value) { return String(value || "Joueur").trim().slice(0, 20) || "Joueur"; }
...
export async function ensureTables(db) {
  await db.prepare(`CREATE TABLE IF NOT EXISTS versus_rooms (
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS versus_players (
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS versus_room_settings (
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS versus_room_options (
...
export async function authenticate(db, roomCode, suppliedToken) {
  ...
  return db.prepare("SELECT * FROM versus_players WHERE room_code = ? AND token = ?")
    .bind(roomCode, String(suppliedToken)).first();
}

export function parseFlags(raw) { try { const value = JSON.parse(raw); return Array.isArray(value) ? value : []; } catch { return []; } }
export function publicFlag(flag, answerMode = "free") {
  if (Array.isArray(flag)) return { image: flag[1] };
  if (flag && typeof flag === "object") {
    const result = typeof flag.prompt === "string"
      ? { prompt: flag.prompt, ...(typeof flag.image === "string" ? { image: flag.image } : {}) }
      : { image: flag.image || flag.url || flag.src || null };
    // Choices are public in MCQ mode; answer and aliases always remain private.
    if (answerMode === "mcq" && Array.isArray(flag.choices)) result.choices = flag.choices;
    return result;
  }
  return { image: null };
}
export function answersFor(flag) {
  ...
  return [flag.answer || flag.name || flag.country, ...(Array.isArray(flag.aliases) ? flag.aliases : [])]
    .filter(value => typeof value === "string" && value.trim());
}
export function answerFor(flag) { return answersFor(flag)[0]; }
export function roomSettings(row) {
  return {
    category: row.category || "flags",

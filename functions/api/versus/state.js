...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const players = await env.DB.prepare(
      ...
      .bind(code)
      .all();
    const list = players.results.map((p) => ({
      playerId: p.slot,
      role: `p${p.slot}`,
      pseudo: p.pseudo,
      hp: p.hp,
      index: p.question_index,
      self: p.slot === self.slot,
    }));
    ...
    return json({
      ...
      role: `p${self.slot}`,
      playerId: self.slot,
      isHost: suppliedToken === room.host_token,
      status: room.status,
      maxPlayers: room.max_players,
      ...settings,
      playerCount: list.length,
      players: list,
      opponentJoined: opponents.length > 0,
      opponentPseudo: opponents[0]?.pseudo || null,
      selfHp: self.hp,
      oppHp: opponents[0]?.hp ?? null,
      selfIndex: self.question_index,
      oppIndex: opponents[0]?.index ?? null,
      winner: room.winner,
      flags: flags.map(makePublic),
      question: flags[self.question_index]
        ? makePublic(flags[self.question_index])
        : null,
    });
  } catch (error) {
    return json(
      { ok: false, error: "Erreur lors de la lecture de l'état du salon." },
      500,
    );
  }
}

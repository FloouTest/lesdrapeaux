  ...
  const DB = {
    prepare(sql) {
      ...
      return {
        ...
        async execute() {
          return /^\s*SELECT/i.test(sql) ? this.all() : this.run();
        },
      };
    },
    async batch(statements) {
      ...
      try {
        ...
        for (const statement of statements)
          result.push(await statement.execute());
        sqlite.exec("COMMIT");
        return result;
      } catch (e) {
...
async function call(handler, env, body, query = "") {
  const request = new Request(
    "https://quiz.test/api/versus/" + query,
    body
      ? {
          method: "POST",
...
test("five-player lobby, host start, private state, replay and elimination guards", async () => {
  ...
  for (let i = 2; i <= 5; i++)
    players.push(
      await call(join, env, { code: host.code, pseudo: "Player" + i }),
    );
  assert.deepEqual(
  ...
  assert.equal(
    (await call(join, env, { code: host.code, pseudo: "Extra" })).statusCode,
    409,
  );
  ...
  const snapshot = await call(
    ...
    env,
    null,
    `?code=${host.code}&token=${host.token}`,
  );
  ...
  const dead = await call(answer, env, {
    code: host.code,
  ...
  env.sqlite
    .prepare("UPDATE versus_players SET question_index = 5 WHERE hp > 0")
    .run();
  const end = await call(answer, env, submission);
  assert.equal(end.status, "finished");
  assert.equal(end.winner, "Host");
  assert.equal(
    (await call(answer, env, { ...submission, index: 1 })).applied,
    false,
  );
});
...
test("capital ranked progress and leaderboard stay independent of flags", async () => {
  ...
  for (const category of ["flags", "capitals"]) {
    const result = await call(ranked, env, {
      pseudo: "Alice",
      category,
      score: 20,
      total: 20,
    ...
    assert.deepEqual(
      list.entries.map((e) => e.pseudo),
      [category],
    );
  }
  assert.equal(
    env.sqlite.prepare("SELECT games_played FROM players").get().games_played,
    1,
  );
  assert.equal(
    env.sqlite.prepare("SELECT games_played FROM ranked_players").get()
      .games_played,
    1,
  );
  for (let i = 0; i < 5; i++)
...
test("capitals migration preserves existing scores", () => {
  const env = database();
  env.sqlite
    .exec(`CREATE TABLE leaderboard (pseudo TEXT, continent TEXT, mode TEXT, score INTEGER, total INTEGER, mistakes INTEGER, seconds INTEGER);
    CREATE TABLE ranked_history (pseudo TEXT, created_at TEXT);`);
  env.sqlite.exec(
    "INSERT INTO leaderboard (pseudo,continent,mode,score,total,mistakes,seconds) VALUES ('Legacy','Monde','qcm',10,10,0,30)",
  );
  env.sqlite.exec(
    readFileSync(
      new URL("../migration_v5_capitals.sql", import.meta.url),
      "utf8",
    ),
  );
  assert.equal(
    env.sqlite.prepare("SELECT category FROM leaderboard").get().category,
    "flags",
  );
  assert.equal(
    env.sqlite.prepare("SELECT score FROM leaderboard").get().score,
    10,
  );
});

test("every quiz country has a capital", () => {
  for (const country of ALL_COUNTRIES)
    assert.ok(CAPITALS[country.name], country.name);
});
